...
# Vérifier les adresses statiques sur les bonnes interfaces (veth1 / veth2)
IP_PC1=$(sudo ip netns exec PC1 ip addr show veth1 2>/dev/null | grep -oP 'inet \K[\d.]+')
IP_PC2=$(sudo ip netns exec PC2 ip addr show veth2 2>/dev/null | grep -oP 'inet \K[\d.]+')

[ "$IP_PC1" = "192.168.10.10" ] && ok "PC1 adresse statique : 192.168.10.10 (veth1)" \
	|| warn "PC1 adresse inattendue sur veth1 : ${IP_PC1:-(aucune)}"
[ "$IP_PC2" = "192.168.20.10" ] && ok "PC2 adresse statique : 192.168.20.10 (veth2)" \
	|| warn "PC2 adresse inattendue sur veth2 : ${IP_PC2:-(aucune)}"
echo ""
...
info "=== ÉTAPE 3 : Règles SNAT (mapping 1:1 IP privée -> IP publique) ==="
...
echo ""
echo "  Pour nettoyer : bash cleanup_static_nat.sh"
echo ""
