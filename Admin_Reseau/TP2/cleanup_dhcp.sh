...
# Sur l'hôte (copies résiduelles)
for f in /tmp/dhcp_lan1.conf /tmp/dhcp_lan2.conf /var/lib/misc/dnsmasq.leases; do
    if [ -f "$f" ]; then
...
# Flush des adresses DHCP sur les interfaces clients
...
# dhcp_pc1 et dhcp_pc2 sont les extrémités côté clients
# Leur suppression entraîne automatiquement la suppression de dhcp1/dhcp2
...
# Si les interfaces sont déjà dans les namespaces, on les supprime de là
