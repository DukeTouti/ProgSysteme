...
# Vérifier que PC1 et PC2 ont bien une IP DHCP
for ns_num in 1 2; do
    IP=$(sudo ip netns exec "PC${ns_num}" ip addr show "dhcp_pc${ns_num}" 2>/dev/null | grep -oP 'inet \K[\d./]+')
    if [ -z "$IP" ]; then
        warn "PC${ns_num} n'a pas d'adresse DHCP sur dhcp_pc${ns_num} — assurez-vous que setup_dhcp.sh a été exécuté"
    else
        ok "PC${ns_num} : adresse DHCP détectée ($IP)"
    fi
done

# Vérifier le forwarding IP actuel sur GATEWAY
FWD=$(sudo ip netns exec GATEWAY cat /proc/sys/net/ipv4/ip_forward 2>/dev/null || echo "0")
if [ "$FWD" != "1" ]; then
    warn "IP forwarding désactivé sur GATEWAY — activation en cours..."
fi
echo ""
...
# Réécrit l'IP source en 209.165.200.226 (IP WAN du GATEWAY)
# pour tout paquet sortant par veth3 depuis LAN1 ou LAN2
...
echo ""
echo "  Pour nettoyer : bash clean_dynamic_nat.sh"
echo ""
