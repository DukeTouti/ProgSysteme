...
info "Nettoyage NAT TP3 (iptables GATEWAY/ISP + règle HOST)..."
sudo ip netns exec GATEWAY iptables -F        2>/dev/null && ok "GATEWAY iptables filter vidé"  || true
sudo ip netns exec GATEWAY iptables -t nat -F 2>/dev/null && ok "GATEWAY iptables nat vidé"     || true
sudo ip netns exec GATEWAY iptables -X        2>/dev/null                                        || true
sudo ip netns exec ISP iptables -F            2>/dev/null && ok "ISP iptables filter vidé"      || true
sudo ip netns exec ISP iptables -t nat -F     2>/dev/null && ok "ISP iptables nat vidé"         || true
HOST_IF=$(ip route 2>/dev/null | awk '/default/ {print $5; exit}')
[ -n "$HOST_IF" ] && sudo iptables -t nat -D POSTROUTING -s 10.0.0.0/30 -o "$HOST_IF" \
    -j MASQUERADE 2>/dev/null && ok "HOST : règle MASQUERADE Internet supprimée" || true
echo ""
